...
never reads .env/.env.local itself, and never accepts any of these values
as a command-line argument):
  ANTHROPIC_API_KEY
  ANTHROPIC_AUTH_SCOPE_MODE   Required for every call (default AND
                               --production-parity). Closed values:
                                 workspace_scoped  -- key scoped to one
                                   workspace; no header sent.
                                 identity_linked   -- key not scoped to one
                                   workspace; ANTHROPIC_WORKSPACE_ID also
                                   required, header sent on every call.
                               See lib/semantic-topic/anthropic-workspace-config.ts
                               (resolveAnthropicAuthConfig) for the full
                               contract.
  ANTHROPIC_WORKSPACE_ID      Required ONLY when ANTHROPIC_AUTH_SCOPE_MODE
                               is identity_linked.
...
async function main(): Promise<number> {
  ...
  const { ANTHROPIC_WORKSPACE_ID_HEADER, resolveAnthropicAuthConfig } = await import('../lib/semantic-topic/anthropic-workspace-config')
  ...
  // ANTHROPIC_AUTH_SCOPE_MODE is required for every call (default AND
  // --production-parity), same fail-closed-before-any-call contract as the
  // API key check just above -- see anthropic-workspace-config.ts's
  // resolveAnthropicAuthConfig() for the full contract, including the
  // 'workspace_scoped' branch that needs no ANTHROPIC_WORKSPACE_ID at all.
  // Never logs the reasonCode's underlying raw value (there isn't one on a
  // failure branch by construction) and never logs a workspace ID value on
  // success either -- only the mode string itself, which is not sensitive.
  const authConfig = resolveAnthropicAuthConfig()
  if (!authConfig.ok) {
    log('error', 'Anthropic auth scope mode/workspace is not configured -- refusing to proceed before any provider call', {
      reasonCode: authConfig.reasonCode,
    })
    return EXIT_CODE.CONFIG_ERROR
  }
  ...
  // workspace header is added via defaultHeaders ONLY in identity_linked
  // mode, exactly mirroring provider-adapter.ts's own buildAnthropicClientOptions
  // -- in workspace_scoped mode the key is entirely omitted, in both
  // default and --production-parity modes (the header is an authentication
  // concern, not a request-shape parity concern, so it does not vary with
  // productionParity, only with authConfig.mode).
  const client = new Anthropic({
    apiKey: process.env.ANTHROPIC_API_KEY,
    timeout: timeoutMs,
    maxRetries: 0,
    ...(authConfig.mode === 'identity_linked' ? { defaultHeaders: { [ANTHROPIC_WORKSPACE_ID_HEADER]: authConfig.workspaceId } } : {}),
  })
  ...
  log('info', 'starting single diagnostic call', {
    ...
    timeoutMs,
    productionParity,
    // The mode string itself ('workspace_scoped' | 'identity_linked') is
    // not sensitive and safe to log as-is -- never the workspace ID value,
    // prefix, length, or fingerprint (Section D's own explicit
    // requirement), which the redactForDisplay() presenter above would
    // fully mask anyway if it somehow appeared under a workspaceId-shaped
    // key, but this field is deliberately never that key.
    authScopeMode: authConfig.mode,
  })
