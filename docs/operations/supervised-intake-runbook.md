...
## 15. Identity-Linked Workspace Header Support v0

**Root cause of the real production `HTTP 400 invalid_request_unbilled` incident**, confirmed via a real diagnostic call and cross-checked against Anthropic's own documentation: the production Anthropic API key is an **identity-linked key** (Personal, scoped to "All workspaces" rather than one specific workspace). Anthropic requires every Messages API request made with a key of this type to carry the workspace it acts in via the `anthropic-workspace-id` request header; omitting it returns exactly the observed error: *"anthropic-workspace-id is required when authenticating with an identity-linked API key; send the id of the workspace this request acts in."* A malformed header value returns 400 `anthropic-workspace-id header must be a valid workspace ID.`; an unknown/inaccessible workspace returns 404. This is documented, expected Anthropic behavior for this key type, not a bug or an account/billing problem — a prior gate's Console read-only audit had already ruled out spend limits and credit exhaustion before this was found.

**Configuration:** a new required environment variable, `ANTHROPIC_WORKSPACE_ID` (format: `wrkspc_` followed by the Console-assigned suffix — find it in **Settings → Workspaces** in the Claude Console, or from the `anthropic-workspace-id` response header of any successful call). It is **not an API secret** — it grants no access on its own — but is still treated as an operator identifier: never logged, never displayed even as a prefix/length/fingerprint, fully masked by `redactForDisplay` (`lib/semantic-topic/operator-cli-security.ts`) wherever a field is named `workspaceId`/`workspace_id`/`ANTHROPIC_WORKSPACE_ID`.

**Fail-closed contract** (`lib/semantic-topic/anthropic-workspace-config.ts`): required for **every** WillViral Anthropic call — production extraction and the diagnostic CLI alike, in both its default and `--production-parity` modes (this is an authentication concern, not a request-shape parity concern). `runShadowExtraction` (`extraction-service.ts`) checks it BEFORE any quota reservation or provider call — right alongside the existing oversized-input check — so a missing or malformed value can never consume a reservation or attempt a call that would fail identically for every remaining item in a batch. The failed item is stored with reason_code `ANTHROPIC_WORKSPACE_CONFIG_ERROR` (migration `082`, a new code distinct from every `081` provider-taxonomy code — those describe the *provider* rejecting a call that was actually sent; this describes a purely local check that runs before any HTTP request is made at all), `retryable = false` (a permanent deployment misconfiguration, not evidence-specific), and stops the whole batch (`stopReasonCode: 'AUTHORIZATION_OR_CONFIG_ERROR'`), exactly like the 081 provider-taxonomy categories.

**Diagnostic CLI:** `scripts/anthropic-provider-diagnostic.ps1` now prompts for `ANTHROPIC_WORKSPACE_ID` via its **own separate** `Read-Host -AsSecureString` call (never reused from the API key prompt), sets it as an environment variable only for the lifetime of the one child process, and clears it in the same `finally` block as the API key — identical protections, never a command-line argument, never written to a file.

**Note for a future production rollout:** applying migration `082` and setting `ANTHROPIC_WORKSPACE_ID` in Vercel Production env are both still-pending, separately authorized steps — not performed by the gate that introduced this support.
