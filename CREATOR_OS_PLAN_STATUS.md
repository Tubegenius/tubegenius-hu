...
| 12 | AI provider layer alap | ⚠️ Fázis A+B+C(2/6) kész (2026-07-08), 4/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új), `MODEL_PRICING` drift javítva, `savePaidResult()` bővítve. **Fázis B**: `opportunity-explain`+`opportunity-similar` átállítva, hiányzó kredit-levonás pótolva. **Fázis C (2/6)**: `viral-score` és `script-extract` `route.ts` átállítva `callAIProvider()`+`extractJson()`-re (a `script-extract` saját, korábban duplikált `extractJson()`-je törölve, a közös verzióra állt); mindkettő `savePaidResult()` hívása kitölti a provider/model/prompt_template_id/prompt_version/estimated_cost mezőket. Élőben tesztelve: `script-extract` valós YouTube URL-lel (Rick Astley teszt-videó) — 1. hívás transcript-alapú elemzés, kredit levonva pontosan 3-at (69→66), 2. hívás (ugyanaz az URL) `paid_results` cache-ből jött, kredit nélkül (`from_paid_result: true`, balance változatlan). A "charge AFTER AI succeeds" sorrendi hiba (audit #12-es tétele) TUDATOSAN VÁLTOZATLAN maradt — ez külön döntés, nem ennek a fázisnak a része. **Következő: Fázis C folytatása** — `video-package` (2 AI hívás) a terv szerinti sorrend alapján. |
