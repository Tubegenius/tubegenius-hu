...
| 12 | AI provider layer alap | ⚠️ Fázis A+B+C(5/6) kész (2026-07-08), 7/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új), `MODEL_PRICING` drift javítva, `savePaidResult()` bővítve. **Fázis B**: `opportunity-explain`+`opportunity-similar` átállítva, hiányzó kredit-levonás pótolva. **Fázis C (5/6)**: `viral-score`, `script-extract`, `video-package`, `video-audit`, `similar-videos` átállítva. A `similar-videos` AI-hívása NEM a route-fájlban volt, hanem egy megosztott libben (`lib/similar-query-expansion.ts`, `generateSimilarVideoQueries()`) — ezt migráltam `callAIProvider()`+`extractJson()`-re, a route maga változatlan (nem is hívott sosem AI-t közvetlenül). Csak ezt az egy libet használja ez a függvény (ellenőrizve), így zéró kockázat más route-okra. Szándékosan NEM adtam hozzá provider/model/cost trackinget a `savePaidResult()`-hoz ennél a route-nál, mert a lib saját in-memory cache-e/fallback-ja miatt a route nem tudná megbízhatóan eldönteni, történt-e valós AI-hívás — ez félrevezető metaadatot eredményezne. Élőben tesztelve: valós lekérdezés-bővítés (hu/en query-k helyesen generálva, log-gal ellenőrizve), 1 kredit levonva pontosan egyszer (60→59), 9 valós videó, cache-reopen ingyenes. **Következő: Fázis C utolsó tétele** — `opportunity` (905 sor, két cache-rendszer). |
