...
| 12 | AI provider layer alap | 🔍 Audit + terv kész (2026-07-08), implementáció nem indult | Teljes 32 route-os audit lefutott, 12 önálló hiba/rés azonosítva (pl. Stripe webhook nem idempotens, `MODEL_PRICING` drift, `opportunity-explain`/`opportunity-similar` nem von le kreditet, `video-packages` nincs kredit/paid_results védve). Részletes route-térkép, service-layer javaslat (`ai-provider-service`, `credit-service`, stb.) és fázisokra bontott terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Következő lépés: Fázis A** (alapréteg, nulla route-módosítás). |
