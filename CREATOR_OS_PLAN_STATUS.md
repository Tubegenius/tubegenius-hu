...
| 12 | AI provider layer alap | ⚠️ Fázis A+B+C(4/6) kész (2026-07-08), 6/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új), `MODEL_PRICING` drift javítva, `savePaidResult()` bővítve. **Fázis B**: `opportunity-explain`+`opportunity-similar` átállítva, hiányzó kredit-levonás pótolva. **Fázis C (4/6)**: `viral-score`, `script-extract`, `video-package`, `video-audit` átállítva `callAIProvider()`+`extractJson()`-re. `video-audit` volt az egyetlen route, ami raw `fetch('https://api.anthropic.com/v1/messages', ...)`-et használt SDK helyett — ez most `callAIProvider()`-re cserélve. A route saját YouTube-fetch reimplementációja (nem a megosztott `lib/youtube-service.ts`-en át) TUDATOSAN VÁLTOZATLAN maradt — a megosztott `youtubeStats()` nem kér `snippet`-et (cím/leírás/tags), a kiterjesztése más route-ok (viral-score, similar-videos) kvótaköltségét is érintené, ez külön, dedikált döntést igényelne, nem ennek a fázisnak a része. Pótoltam egy hiányzó `logUsage()` hívást is (a többi migrált route-tól eltérően ez korábban sosem logolt AI-használatot). Élőben tesztelve valós YouTube URL-lel (Rick Astley teszt-videó): 4 kredit levonva pontosan egyszer (64→60), valós magyar diagnózis/hook-javaslat generálva, cache-reopen ingyenes. **Következő: Fázis C folytatása** — `opportunity` és `similar-videos` (a 2 legnagyobb, legkockázatosabb route, dupla cache-rendszerrel). |
