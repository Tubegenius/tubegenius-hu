...
- AI input költségvédelem: új közös `lib/api-input-validation.ts`, 300 karakteres backend témakorláttal. Bekötve még a cache/lock/külső API/AI-hívás előtt 9 fő eszközbe: Opportunity Engine, Similar Videos, Viral Score, Video Package, Title Studio, Thumbnail Studio, SEO Optimizer, Keyword Research, Content Gap. A korábban korlátlan kliensszöveg többé nem okozhat fix kreditért aránytalan token- és külső API-terhelést; túl hosszú input tiszta 400-as magyar hibát kap.
- Production dependency hardening: `npm audit --omit=dev` 2 moderate PostCSS sérülékenységet talált a Next 15.5.20 beágyazott `postcss@8.4.31` példányában. Next 15-höz nincs újabb javított patch, a téves npm auto-fix Next 9-re downgrade-elt volna. Célzott npm override a Next → PostCSS élre (`8.5.15`), root PostCSS `8.5.19`; audit eredmény: **0 sérülékenység**, production build változatlanul sikeres.
- HTTP security headers: `next.config.js` globálisan küldi az `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin` és szűk `Permissions-Policy` fejléceket. Titokscan során valódi commitolt kulcs nem került elő; csak `.env.example` placeholder és README változónevek. CSP tudatosan nincs vakon bekapcsolva, mert a Next inline runtime-jához előbb nonce-alapú CSP architektúra kell.
- Cross-tool credit race lezárva: a meglévő `in_flight_requests` lock most nem csak azonos tool+input kombinációra, hanem userenként egy közös aktív fizetős/generáló műveletre érvényes. Az összes kreditlevonó route már ezt a helpert használja, ezért két eltérő, párhuzamos kérés sem futhat el ugyanabból az előzetesen elégséges balance-ból az AI-hívásig. A második kérés AI-/külső API-költség és kreditlevonás előtt tiszta 409-et kap; más userek párhuzamossága változatlan. Nincs új migráció, a 027-es éles tábla/index elegendő.
