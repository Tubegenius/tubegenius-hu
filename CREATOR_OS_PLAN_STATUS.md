...
| 12 | AI provider layer alap | ⚠️ Fázis A+B+C(1/6) kész (2026-07-08), 3/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új), `MODEL_PRICING` drift javítva, `savePaidResult()` bővítve. **Fázis B**: `opportunity-explain`+`opportunity-similar` átállítva, hiányzó kredit-levonás pótolva. **Fázis C (1/6 — viral-score)**: `app/api/viral-score/route.ts` `new Anthropic()`+kézi JSON-parse lecserélve `callAIProvider()`+`extractJson()`-re; a sikeres (kredit-terhelő) `savePaidResult()` hívás mostmár kitölti a `provider`/`model`/`prompt_template_id`/`prompt_version`/`estimated_cost` mezőket — ez az ELSŐ route, ahol ezek a migráció-021-es oszlopok ténylegesen megtelnek. Élőben tesztelve valós topic-kal (`force_refresh: true`): kredit levonva (70→69), score/verdict/recommendation helyes, `paid_results` mentés hibamentes (nincs a szokásos "[ViralScore] KRITIKUS" log). A dual-cache (legacy `viral_score_searches` + `paid_results`) és a Video Idea proof-signal logika VÁLTOZATLAN maradt — csak az AI-hívás rétege cserélődött. **Következő: Fázis C folytatása** — `script-extract` a terv szerinti sorrend alapján. |
