...
| 12 | AI provider layer alap | ⚠️ Fázis A+B+C(3/6) kész (2026-07-08), 5/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új), `MODEL_PRICING` drift javítva, `savePaidResult()` bővítve. **Fázis B**: `opportunity-explain`+`opportunity-similar` átállítva, hiányzó kredit-levonás pótolva. **Fázis C (3/6)**: `viral-score`, `script-extract`, `video-package` átállítva `callAIProvider()`+`extractJson()`-re. A `video-package`-nél talált egy fontos részlet: a route saját `extractJson()`-je extra lépést tartalmazott (sortörés-szűrés JSON string értékeken belül, mert Claude időnként betesz egy sortörést a "narration" mezőbe, ami eltörné a `JSON.parse`-t) — ezt a lépést ÁTEMELTEM a közös `lib/services/ai-provider-service.ts` `extractJson()`-be, így mind a 3 route (és minden jövőbeli) megkapja ezt a robusztusságot, nem csak video-package. `video-package`-nél 2 AI-hívás van (core=Sonnet, packaging=Haiku) egy paid_result sorban — a `provider`/`model` mezőknél a `chargeFeature()` már meglévő "combined" konvencióját követtem (`model: 'combined'`), `estimated_cost` a 2 hívás összege. Élőben tesztelve: valós generálás (30sec Shorts), 2 kredit levonva (66→64), narration 445 karakter, NINCS benne sortörés (a sanitize működik), 5 scene, 5 title variáció, cache-reopen ingyenes. **Következő: Fázis C folytatása** — `video-audit` (raw-fetch → SDK migrálás is). |
