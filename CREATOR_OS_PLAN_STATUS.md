...
| 12 | AI provider layer alap | ⚠️ Fázis A+B kész (2026-07-08), 2/9 AI-hívó route átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A**: `lib/services/ai-provider-service.ts` (új — `callAIProvider()`, egységes `extractJson()`), `lib/credits.ts` `MODEL_PRICING` drift-hiba javítva, `paid-results-service.ts` `savePaidResult()` bővítve az 5 új mezővel. **Fázis B**: `app/api/opportunity-explain` és `app/api/opportunity-similar` átállítva az új rétegre; ÉS a user döntése alapján bekötve a hiányzó kredit-levonás is (`hasEnoughCredits`+`chargeFeature('opportunity_explain', 1)`) — korábban ez a 2 route ingyenes volt egy hiba miatt, mostantól 1-1 kreditbe kerül. Frontend (`app/dashboard/opportunities/page.tsx`): 402-es hibaüzenet most már látható a userének mindkét hívásnál (korábban `handleReplace`-nél csendben elnyelte a hibát). Élőben tesztelve valós Claude-hívással: mindkét route helyesen levon 1-1 kreditet (72→71→70), hibamentes. **Következő: Fázis C** (nagyobb, kredit-terhelő route-ok egyenként — viral-score elsőként). |
