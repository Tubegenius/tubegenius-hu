...
| 12 | AI provider layer alap | ⚠️ Fázis A kész (2026-07-08), route-ok még nincsenek átállítva | Audit+terv: [AI_PROVIDER_LAYER_REFACTOR_PLAN.md](AI_PROVIDER_LAYER_REFACTOR_PLAN.md). **Fázis A elkészült**: `lib/services/ai-provider-service.ts` (új — `callAIProvider()`, egységes `extractJson()`), `lib/credits.ts` `MODEL_PRICING` drift-hiba javítva (elavult modellnevek → `lib/models.ts` MODELS-re hivatkozva), `lib/paid-results/paid-results-service.ts` `savePaidResult()` bővítve a migráció 021 5 mezőjével (provider/model/prompt_template_id/prompt_version/estimated_cost + `linked_video_idea_id`) — mind opcionális, visszafelé kompatibilis, egyetlen route sem hívja még. `tsc`+`build` zöld. **Egyetlen élő route sincs átírva** — a route-onkénti átállás (Fázis B: opportunity-explain/opportunity-similar) a következő lépés. |
