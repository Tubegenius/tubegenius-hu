...
describe('scripts/execute-approved-lifecycle-transition.ts CLI boundary', () => {
  ...
  it('accepts exactly the documented flags -- no more, no fewer (schema keys + the separately-handled --help)', () => {
    const schemaBlockMatch = cliSrc.match(/EXECUTOR_FLAG_SCHEMA:[\s\S]*?\{([\s\S]*?)\}/)
    expect(schemaBlockMatch).not.toBeNull()
    const schemaFlags = schemaBlockMatch ? [...schemaBlockMatch[1].matchAll(/'(--[a-z-]+)':/g)].map((m) => m[1]) : []
    expect(new Set(schemaFlags)).toEqual(new Set(['--dry-run', '--apply', '--review-request-id', '--confirm-production-project-ref']))
