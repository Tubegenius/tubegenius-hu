          ...
          {check.reason === 'insufficient_credits' && (
            <div className="rounded-lg px-4 py-3 mb-4" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span style={{ color: '#EF4444' }} className="font-semibold">{formatCreditAmount(check.cost)} kredit</span>
              </div>
              <div className="flex justify-between text-sm">
                <span style={{ color: '#CBD5E1' }}>Jelenlegi kredited</span>
                <span style={{ color: '#EF4444' }} className="font-semibold">{formatCreditAmount(check.currentCredits)}</span>
              </div>
            </div>
          )}
      ...
      <div ref={containerRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId}
        ...
        <div className="rounded-lg px-4 py-3 mb-4" style={{ background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.15)' }}>
          <div className="flex justify-between text-sm mb-1">
            ...
            <span style={{ color: '#3B82F6' }} className="font-semibold">{formatCreditAmount(check.cost)} kredit</span>
          </div>
          <div className="flex justify-between text-sm mb-1">
            <span style={{ color: '#CBD5E1' }}>Jelenlegi kredited</span>
            <span style={{ color: '#F8FAFC' }} className="font-semibold">{formatCreditAmount(check.currentCredits)}</span>
          </div>
          <div className="flex justify-between text-sm pt-2" style={{ borderTop: '1px solid rgba(59,130,246,0.1)' }}>
            ...
            <span style={{ color: '#22C55E' }} className="font-semibold">{formatCreditAmount(check.remainingCreditsAfterRun)}</span>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={loading} className="flex-1 py-2 rounded-lg text-sm font-medium" style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', color: '#3B82F6' }}>
            {loading ? 'Feldolgozás...' : `Futtatás ${formatCreditAmount(check.cost)} kreditért`}
          </button>
        </div>
      </div>
    </div>
