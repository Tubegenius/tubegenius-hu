...
interface DashboardSummary {
  ...
  fact_safety: { verified: number; verified_with_limits: number; insufficient_sources: number; standard_news: number; high_risk: number }
...
function EmptyState({ text }: { text: string }) {
  return <p className="text-sm py-4 text-center" style={{ color: '#64748B' }}>{clean(text)}</p>
}
...
function StatusBadge({ status }: { status: string }) {
  const displayStatus = clean(status)
  const meta = STATUS_BADGE[displayStatus] || STATUS_BADGE[status] || { color: '#93C5FD', bg: 'rgba(139,155,180,0.12)' }
  return (
              ...
              <table className="w-full text-xs border-collapse">
                ...
                <tbody>
                  {recent_activity.map((item, i) => {
                    ...
                    const trend = item.trend_status ? TREND_BADGE[item.trend_status] : null
                    return (
                      <tr key={i} className="transition-colors hover:bg-white/[0.02]" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                        <td className="px-1 py-2.5 max-w-[180px]">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="truncate" style={{ color: '#F8FAFC' }}>{clean(item.topic)}</span>
                          </div>
                        </td>
                        <td className="px-1 py-2.5" style={{ color: '#94A3B8' }}>{ACTIVITY_TYPE_LABEL[item.type]}</td>
                        <td className="px-1 py-2.5">
                          {item.score != null ? <span className="font-semibold" style={{ color: scoreColor(item.score) }}>{item.score}</span> : item.status ? <StatusBadge status={item.status} /> : <span style={{ color: '#64748B' }}>-</span>}
                        </td>
                        <td className="px-1 py-2.5">
                          {trend ? (
                            <div className="flex items-center gap-1.5">
                              {item.view_history.length >= 2 && <Sparkline values={item.view_history} width={40} height={16} color={trend.color} />}
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0" style={{ color: trend.color, background: trend.bg }}>
                                <i className={`ti ${trend.icon}`} style={{ fontSize: '10px' }} />
                                {trend.label}
                              </span>
                            </div>
                        ...
                        <td className="px-1 py-2.5 text-right">
                          <Link href={item.href} style={{ color: '#3B82F6' }} title="Megnyitás">
        ...
        <div className="flex flex-col gap-4">
          {audits.avg_score != null && (
            <div className="p-5 text-center" style={PANEL_STYLE}>
              ...
              <div className="text-3xl font-black" style={{ color: scoreColor(audits.avg_score) }}>{audits.avg_score}</div>
              <p className="text-xs mt-1" style={{ color: '#64748B' }}>{audits.total} lefuttatott audit alapján</p>
            </div>
          )}
          ...
          <div className="p-5" style={PANEL_STYLE}>
            ...
            <p className="text-xs leading-relaxed" style={{ color: '#94A3B8' }}>{clean(content_direction_insight)}</p>
          </div>
          ...
          <div className="p-5" style={PANEL_STYLE}>
            ...
            {packages.total === 0 ? <EmptyState text="Még nincs generált csomag." /> : (
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between"><span style={{ color: '#94A3B8' }}>Ellenőrzött</span><span className="font-semibold" style={{ color: '#4ADE80' }}>{fact_safety.verified}</span></div>
                <div className="flex items-center justify-between"><span style={{ color: '#94A3B8' }}>Ellenőrzött, korlátokkal</span><span className="font-semibold" style={{ color: '#FBBF24' }}>{fact_safety.verified_with_limits}</span></div>
                <div className="flex items-center justify-between"><span style={{ color: '#94A3B8' }}>Kevés forrás</span><span className="font-semibold" style={{ color: '#F87171' }}>{fact_safety.insufficient_sources}</span></div>
                <div className="flex items-center justify-between pt-2 mt-1 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}><span style={{ color: '#94A3B8' }}>Standard hír / magas kockázat</span><span className="font-semibold" style={{ color: '#CBD5E1' }}>{fact_safety.standard_news} / {fact_safety.high_risk}</span></div>
              </div>
            )}
          </div>
          ...
          <div className="p-5" style={PANEL_STYLE}>
            ...
            {projects.length === 0 ? <EmptyState text="Még nincs videócsomag vagy audit." /> : (
                ...
                {projects.map((p, i) => {
                  ...
                  return (
                    <Link key={i} href={p.href} className="flex items-center gap-2.5 py-1.5 -mx-1 px-1 rounded-lg transition-colors hover:bg-white/[0.03]">
                      ...
                      <span className="text-xs flex-1 min-w-0 truncate" style={{ color: '#CBD5E1' }}>{clean(p.title).replace(/^(Videócsomag készült: |Audit lefuttatva: )/, '')}</span>
            ...
            <Link href="/dashboard/opportunities" className="block w-full text-center text-xs font-semibold py-2.5 rounded-xl transition-all hover:-translate-y-0.5" style={{ background: 'linear-gradient(135deg, #3B82F6, #8B5CF6)', color: '#fff', boxShadow: '0 0 20px rgba(59,130,246,0.25)' }}>
